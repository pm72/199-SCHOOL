import math

# floor and ceil

##print(math.floor(5.6))        # 5
##print(math.floor(5.3))        # 5
##print(math.floor(5))          # 5
##
##print()
##
##print(math.floor(-5.6))       # -6
##print(math.floor(-5.3))       # -6
##print(math.floor(-5))         # -5
##
##print()
##
##print(math.ceil(5.6))         # 6
##print(math.ceil(5.3))         # 6
##print(math.ceil(5))           # 5
##
##print()
##
##print(math.ceil(-5.6))        # -5
##print(math.ceil(-5.3))        # -5
##print(math.ceil(-5))          # -5




### power and square toot
##
##print(math.pow(5, 3))
##print(math.sqrt(25))
##
### alternative methods
##print(5 ** 3)
##print(25 ** 0.5)
##print(25 ** (1/2))
##print(8 ** 0.33333)
##print(8 ** (1/3))



### Factorial of a number

### 5! = 1 * 2 * 3 * 4 * 5
### 5! = 5 * 4 * 3 * 2 * 1
##print(math.factorial(5))
##print(math.factorial(15))
##print(math.factorial(153))


### minimum, maximum
##print(min(5, 89, 21, 51, -5.69, 10.02, -4.01, 11.58))
##print(max(5, 89, 21, 51, -5.69, 10.02, -4.01, 11.58))
##
### nagative number to positive
##print(abs(-100))
##print(abs(-89.14))
